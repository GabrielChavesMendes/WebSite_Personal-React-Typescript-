...
export function Contato() {
  ...
  return (
    <section id="contatos" className="relative py-20 px-8 max-w-7xl mx-auto w-full z-20">
      
      {/* TÍTULO */}
      <div className="mb-12">
        <h2 className="text-[60px] md:text-[90px] font-black texto-vazado tracking-widest leading-none -translate-x-20">
          CONTATO
        </h2>
      </div>

      {/* DIVISÃO EM DUAS COLUNAS */}
      <div className="flex flex-col md:flex-row justify-between gap-45">
        
        {/* LADO ESQUERDO: Formulário e Info Direta */}
        <div className="w-full md:w-3/5 flex flex-col gap-10 -translate-x-20">
          
          {/* Caixa de Comentário */}
          <div>
            <h3 className="text-white font-bold text-xl mb-4">Deixe um comentário</h3>
            <textarea 
              rows={5}
            ...
            {/* BOTÃO "VER COMENTÁRIOS" (Adicionado aqui!) */}
            <button 
              onClick={() => setIsModalOpen(true)}
              className="px-8 py-3 bg-transparent border-2 border-[#6d28d9] text-white font-bold rounded-xl transition-all duration-300 hover:bg-[#6d28d9] hover:-translate-y-1 hover:shadow-[0_5px_15px_rgba(109,40,217,0.4)]"
            >
              Ver comentários
            </button>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3">
              <li className="flex items-center gap-3 text-white font-medium text-lg">
                {/* Ícone de Email colorido com o M do Gmail */}
          ...
          <ul className="flex flex-col gap-8">
            ...
            <li>
              <a href="#" className="flex items-center gap-6 group">
                ...
                <span className="text-white text-xl font-medium group-hover:text-purple-400 transition-colors">@omalheiros</span>
              </a>
            </li>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          ...
          <div className="bg-[#121212] border border-purple-800 rounded-3xl p-8 w-full max-w-xl relative shadow-[0_0_30px_rgba(109,40,217,0.3)]">
